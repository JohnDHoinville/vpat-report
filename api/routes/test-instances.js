...
router.get('/', authenticateToken, async (req, res) => {
    try {
        ...
        if (search) {
            paramCount++;
            query += ` AND (
                LOWER(wr.criterion_number) LIKE LOWER($${paramCount}) OR
                LOWER(wr.title) LIKE LOWER($${paramCount}) OR
        ...
        if (search) {
            countParamCount++;
            countQuery += ` AND (
                LOWER(wr.criterion_number) LIKE LOWER($${countParamCount}) OR
                LOWER(wr.title) LIKE LOWER($${countParamCount}) OR
...
/**
 * GET /api/test-instances/:id/audit-log
 * Get audit log for a specific test instance
 */
router.get('/:id/audit-log', authenticateToken, async (req, res) => {
    try {
        ...
        console.error('Error fetching test instance audit log:', error);
        res.status(500).json({
            success: false,
            error: 'Failed to fetch test instance audit log',
            details: error.message
        });
    }
});
